...
> [!TIP]
> 目前所使用的資料庫支援全球國家 IP 段，若上述可選區域分流清單中未包含您所需要的國家代碼，可在設定檔後自行新增對應國家的 GEOIP 規則。
...
5. **域名規則與 IP 網路段規則的使用說明**

   ```bash
   建議優先透過 DOMAIN-SUFFIX 域名規則識別具體服務與業務歸屬，將 IP-CIDR 與 IP-CIDR6 規則作為網路承載與基礎設施歸屬的識別參考。大型平台網路段可能承載第三方服務或開源專案資源，不等同於具體服務或業務歸屬，應避免將服務身份與承載網路混淆。
   ```
...
[@ESET](https://github.com/eset) 👉 提供 [malware-ioc](https://github.com/eset/malware-ioc) 公共倉庫資料，作為 `Malware_IOC_Block.list` 中惡意軟體 IOC 規則的重要資訊源([BSD 2-Clause License](https://github.com/eset/malware-ioc/blob/master/LICENSE))。

[微步在線](https://s.threatbook.com/cybercrime/silverfox) 👉 提供銀狐木馬相關公開威脅情報與攻擊活動分析資料，作為 `Phishing_Block.list` 與 `Backdoor_Block.list` 中銀狐相關規則的重要資訊源。
...
本專案採用 [Apache License 2.0](https://github.com/Thoseyearsbrian/Aegis/blob/main/LICENSE) 授權。

你可以在遵守授權條款的前提下，自由使用、修改與分發本專案內容，包括用於商業用途。

使用或再分發本專案時，請保留原始作者署名、授權聲明、關鍵註釋與來源資訊，以保障規則可溯源、可審計與可驗證。

建議透過遠端引用方式分發與更新規則。直接複製規則內容並長期靜態使用，可能導致版本漂移、錯過安全更新、威脅情報過期，並增加維護成本與信任風險。

本專案在適用情況下使用 [GPG](https://gnupg.org) 簽名提交，以協助驗證專案變更的真實性與完整性。
