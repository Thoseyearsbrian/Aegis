...
[ThreatBook 微步在線](https://s.threatbook.com/cybercrime/silverfox) 👉 提供銀狐木馬相關公開威脅情報與攻擊活動分析資料，作為 `Phishing_Block.list` 與 `Backdoor_Block.list` 中銀狐相關規則的重要資訊源。
...
此外，Aegis 專案已啟用 [GPG](https://gnupg.org) 簽名機制，以確保專案程式碼來源真實可信、未被竄改。你可透過 GPG 簽名驗證每一次提交操作的完整性，從而獲得更高的安全保障。

> [!NOTE]
> 為確保規則集的安全性、完整性與可審計性，並保障長期維護與持續校對能力，建議透過遠端引用方式分發與使用本專案規則集。

> [!WARNING]
> 直接複製規則內容並長期靜態使用，可能帶來以下風險：版本漂移與規則失效，無法及時取得安全更新，錯過新增威脅情報或誤判修正，增加維護成本與信任風險。
