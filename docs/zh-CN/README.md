...
[@ESET](https://github.com/eset) 👉 提供 [malware-ioc](https://github.com/eset/malware-ioc) 公共仓库数据，作为 `Malware_IOC_Block.list` 中恶意软件 IOC 规则的重要信息源([BSD 2-Clause License](https://github.com/eset/malware-ioc/blob/master/LICENSE))。

[微步在线](https://s.threatbook.com/cybercrime/silverfox) 👉 提供银狐木马相关公开威胁情报与攻击活动分析资料，作为 `Phishing_Block.list` 与 `Backdoor_Block.list` 中银狐相关规则的重要信息源。
...
本项目采用 [Apache License 2.0](https://github.com/Thoseyearsbrian/Aegis/blob/main/LICENSE) 授权。

你可以在遵守许可证条款的前提下，自由使用、修改与分发本项目内容，包括用于商业用途。

使用或再分发本项目时，请保留原始作者署名、授权声明、关键注释与来源信息，以保障规则可溯源、可审计与可验证。

建议通过远程引用方式分发与更新规则。直接复制规则内容并长期静态使用，可能导致版本漂移、错过安全更新、威胁情报过期，并增加维护成本与信任风险。

本项目在适用情况下使用 [GPG](https://gnupg.org) 签名提交，以帮助验证项目变更的真实性与完整性。
