...
[Surge](https://nssurge.com) 个人数字防火墙规则集，专注于识别应用层与传输层的潜在通信威胁，涵盖 [DNS 污染](https://zh.wikipedia.org/wiki/域名服务器缓存污染)、[APT 攻击源](https://zh.wikipedia.org/wiki/高级长期威胁)、[SDK 回传监听](https://en.wikipedia.org/wiki/Software_development_kit)、[后门通信](https://en.wikipedia.org/wiki/Back_door)、[PCDN 链路通信](https://en.wikipedia.org/wiki/P2P_caching)、[C2 控制器](https://zh.wikipedia.org/wiki/殭屍網絡)等潜在通信行为，同时扩展对全球主流广告、行为追踪与成人内容平台的域名识别，帮助用户在 iOS 与 macOS 本地实现流量的精准识别与分类，并根据自身需求自主设定流量策略。
...
Aegis 遵循 [语义化版本](https://semver.org/lang/zh-CN/) 控制规范：`vMAJOR.MINOR.PATCH`。

示例：Aegis v3.0.1，其中 MAJOR=3、MINOR=0、PATCH=1。

MAJOR（X）版本（重大更新）：涉及结构调整或兼容性变化 👉 必须重新下载配置，并重新添加节点。

MINOR（Y）版本（新增功能）：涉及新增策略组、新模块或新能力 👉 建议重新下载配置，并重新添加节点。

PATCH（Z）版本（修复更新）：涉及规则修复、注释补全或小幅优化 👉 无需重新下载配置，只需更新外部资源即可。
...
> [!WARNING]
> 请根据网络环境选择对应配置。如您的网络环境原生支持 IPv6，请使用 IPv6 配置；否则请使用 IPv4 配置。
> 严禁混用，否则将导致请求异常或 DNS 解析失败。
...
> [!TIP]
> 若上述可选区域分流列表中未包含您需要的国家代码，可在配置文件后自行添加对应国家的 GEOIP 规则。
> 当前所使用的数据库支持全球国家 GEOIP 查询，可按 ISO 国家代码自由扩展。
...
> [!NOTE]
> - 优先使用 `RULE-SET` 或官方配置订阅链接；
> - 避免直接复制粘贴全部规则内容后长期静态使用；
> - 如需二次封装或整合，请保留原始引用来源，确保用户能够获取最新规则数据。

> [!WARNING]
> 直接复制规则内容并长期静态使用，可能带来以下风险：
> - 版本漂移与规则失效；
> - 无法及时获取安全更新；
> - 错过新增威胁情报或误判修复；
> - 增加维护成本与信任风险。
