...
> [!NOTE]
> 當前版本是 Aegis v2.0.0，上一版本是 Aegis v1.3.4，此次為 MAJOR 更新（MAJOR=2），必須重新下載設定，並重新添加節點，會自動更新外部資源。
> 
> 當前版本是 Aegis v2.3.0，上一版本是 Aegis v2.2.4，此次為 MINOR 更新（MINOR=3），建議重新下載設定，並重新添加節點，會自動更新外部資源。
> 
> 當前版本是 Aegis v3.3.4，上一版本是 Aegis v3.3.3，此次為 PATCH 更新（PATCH=4），不用重新下載設定，只需更新外部資源即可。
...
```bash
# GEOIP,US,Proxy # 美國區域流量走代理（啟用後可存取美國站點）

# GEOIP,GB,Proxy # 英國區域流量走代理（啟用後可存取英國站點）

# GEOIP,FR,Proxy # 法國區域流量走代理（啟用後可存取法國站點）

# GEOIP,DE,Proxy # 德國區域流量走代理（啟用後可存取德國站點）

# GEOIP,RU,Proxy # 俄羅斯區域流量走代理（啟用後可存取俄羅斯站點）

# GEOIP,EU,Proxy # 歐盟區域流量走代理（啟用後可存取多數歐洲站點）

# GEOIP,AU,Proxy # 澳洲區域流量走代理（啟用後可存取澳洲站點）

# GEOIP,JP,Proxy # 日本區域流量走代理（啟用後可存取日本站點）

# GEOIP,KR,Proxy # 韓國區域流量走代理（啟用後可存取韓國站點）

# GEOIP,SG,Proxy # 新加坡區域流量走代理（啟用後可存取新加坡站點）
```
