...
> [!NOTE]
> 當前版本是Aegis v2.0.0,上一版本是Aegis v1.3.4,此次就是MAJOR更新(MAJOR=2),必須重新下載設定，並重新添加節點,會自動更新外部資源
> 
> 當前版本是Aegis v2.3.0,上一版本是Aegis v2.2.4,此次就是MINOR更新(MINOR=3),建議重新下載設定，並重新添加節點,會自動更新外部資源
> 
> 當前版本是Aegis v3.3.4,上一版本是Aegis v3.3.3,此次就是PATCH更新(PATCH=4),不用重新下載設定，只需更新外部資源即可
...
Aegis 預設開啟: GEOIP,CN,DIRECT # GEOIP 匹配中國大陸

### 可選區域分流功能

為一般使用者提供簡化的國際分流能力，減少存取受阻情況，Aegis 支援可選區域分流功能，支援以下等地區的分流能力:
...
> 目前所使用的資料庫支援全球國家 GEOIP 查詢，可依 ISO 國家代碼自由擴充。
